...
func main() {
	...
	attendanceSvc, err := attendance.NewService(ctx, mongoClient, rbac)
	if err != nil {
		log.Fatal().Err(err).Msgf("failed to initialize Attendance Service")
	}
	promotionsSvc, err := promotions.NewService(ctx, mongoClient, rbac)
	if err != nil {
		log.Fatal().Err(err).Msgf("failed to initialize Promotions Service")
	}
	dashboardSvc, err := dashboard.NewService(ctx, mongoClient, rbac)
	if err != nil {
		log.Fatal().Err(err).Msgf("failed to initialize Dashboard Service")
	}
	membersSvc, err := members.NewService(ctx, mongoClient, rbac)
	if err != nil {
