...
func (s *Service) ProcessPut(ctx context.Context, req events.APIGatewayProxyRequest) (events.APIGatewayProxyResponse, error) {
	...
	var result GymRequest
	if err := mongoext.UpdateByID(ctx, s.Collection, id, gymRequest, &result, nil); err != nil {
...
func (s *Service) updateRequestTransaction(ctx context.Context, payload *GymRequest, id string) (*GymRequest, error) {
	...
	result, err := s.WithTransaction(ctx, func(sessCtx mongo.SessionContext) (interface{}, error) {
		...
		log.Info().Msgf("Update GymRequest result: %v", result)

		if payload.Status != RequestApproved {
			return result, nil
		}

		// The request was approved: update the student profile's gym_associations field.
		...
		upsertResult := profiles.Profile{}
		filter := bson.M{
			"cognito_id": payload.RequestorID,
		}
		...
		log.Info().Msgf("Successfully synced user's profile with ")
		return result, nil
	}, transactionOptions)
